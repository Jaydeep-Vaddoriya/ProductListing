...
namespace ProductManagement.Provider
{
    ...
    public static class FunctionProvider
    {
        public static object GetPropertyValue<T>(this T data, string propertyName)
            => data.GetType().GetProperty(propertyName)?.GetValue(data, null);

        public static object SetPropertyValue<T>(this T data, string propertyName, object value)
        {
            data.GetType().GetProperty(propertyName)?.SetValue(data, value, null);
            return data;
        }
    }
